...
From Qf1 we can then construct a second order question \ref{Qf2}:
	...
	\end{principle}
	\footnotetext{If value \textit{simpliciter} were inappropriate (e.g.\ if the measure of value in question were relative), similar questions could still be constructed by replacing `value' with `relative value' or `position in a series of all questions in value-order' for example.} % 'similar' isn't quite right here

The answer to \ref{Qf2} will provide all the information the answer to Qf1 would have (that is, it will tell us A) \emph{and} will tell us the value of Qf1.
