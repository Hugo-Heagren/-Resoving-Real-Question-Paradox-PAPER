% I need to decide how I'm going to present the references. Using math mode? With subscripts? Same for brackets.
...
% Taken together, the fact that there is an upper limit on the value a question can provide and that there is an infinite series of such questions means that while the value increases, the difference between the values of consecutive questions must decrease.
...
% At some point $n$ in the series the difference between the value of the question $Q_n$ and the maximum will become negligibly small.
...
% final note: this all assumes that we are using the very strict understanding of 'best' outlined above.
% but we don't need to. In fact, we rarely do. Say a sportsperson has come second in their event in every olympic games for the last 10 years, as first place winners have come and gone. At any point in time it is true that there is a person who is better than them in their discipline, (so they are not \emph{the} best) but we would not hesitate to call them \emph{one of the best}.
